...
class Crab:
    def __init__(self, x: int, y: int, __v_x: int, __v_y: int) -> None:
        ...
        self.__dead = False
        self.__angry = False
        self.__time_since_last_angry = 0
    ...
    def __collide_enemies(self, enemies: list):
        for enemy in enemies:
            if enemy is not self and self.__is_colliding(enemy):
                ...
                # Reverse the crab's direction and update looking direction
                self.__v_x = -self.__v_x
    ...
    def __collide_coins(self, coins: list):
        for coin in coins:
            if self.__is_colliding(coin):
                # Adjust position and direction when colliding with coins
                if self.x < coin.x:
    ...
    def __gravity_push(self):
        # Apply gravity force to the crab if it is not at the bottom of the screen
        if self.y < pyxel.height:
            self.__v_y += c.gravity

    def __turn_upside(self, player):
        if (
            abs((player.y - player.height) - (self.y + self.height)) < 2
            and abs(player.x - self.x) < 14
        ):
            if (
                self.__time_since_last_angry == 0
                or self.__time_since_last_angry > 20
            ):
                self.__angry = True
    ...
    def __update_animations(self, player):
        if self.__angry and self.__is_colliding(player):
            ...
            turning_frames = (
                [c.s_crab_turning_r1, c.s_crab_turning_r2]
                if self.__looking_right
                else [c.s_crab_turning_l1, c.s_crab_turning_l2]
            )
            frame_index = int(
                (
                    (c.turning_animation_frames - self.__turning_frames)
                    / c.turning_animation_frames
                )
                * len(turning_frames)
            )
            ...
            self.__v_y = -2.5
            if self.__angry:
                self.sprite = c.s_crab_angry_1
            else:
                self.sprite = c.s_crab_die_1
